...
const Difference: React.FC<Props> = ({ clearAll }) => {
  ...
  const getNextMonths = (current_month: string, data: any): string[] => {
    ...
    const countData = Object.keys(data?.difference).length;
    console.log("count:", countData)
    // Check if current_month is provided and valid, otherwise use the current date


    // Get the values of the cotango object and skip the first field
    const originalValues = Object.values(data?.difference).slice(0, countData);
  ...
  //   const transformed = [];
  //   let index = 0;
  //   for (const key in data.cotango) {
  //     if (
  //       data.cotango.hasOwnProperty(key) &&
  //       data.difference.hasOwnProperty(key)
  //     ) {
  //       const labels = getNext7Months(data.current_month);
  //       transformed.push({
  //         label: labels[index],
  //         contango: parseFloat(data.cotango[key]).toFixed(2),
  ...
  const transformData = (data: any) => {
    ...
    // Get the keys of difference and skip the first key
    ...
    differenceKeys.forEach((key, index) => {
      // Check if it's the last element
      if (index === differenceKeys.length - 1) {
  ...
  const displayedStats = clearAll
    //@ts-ignore
    ? stats.map(stat => ({ ...stat, difference: null }))
    : stats;

  return (
    <div className="w-full flex gap-4">
      <StatHeading title="Difference" />
      {loading && firstLoad ? (
        <div className="flex items-center justify-center w-full">
          <LoaderIcon />
        </div>
        ...
        <div
          className={`flex overflow-x-auto border w-full dark:border-border border-lightBorder rounded ${theme === "dark" ? "customScrollbar" : "customScrollbarLight"
            }`}
        >
